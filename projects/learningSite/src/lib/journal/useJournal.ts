...
import { readJournal, STORAGE_KEY } from "./store";
...
// actually changes, but readJournal() re-parses JSON on every call —
...
let cachedJournal: Journal | undefined;

function getSnapshot(): Journal {
  const raw = localStorage.getItem(STORAGE_KEY);
  if (raw !== cachedRaw || cachedJournal === undefined) {
    cachedRaw = raw;
    cachedJournal = readJournal();
  }
  return cachedJournal;
}

// Server has no localStorage — render nothing journal-dependent until the
// client snapshot resolves, rather than guessing and risking a hydration
// mismatch. Same pattern as useNotes/useProgress.
function getServerSnapshot(): null {
  return null;
}

export function useJournal(): Journal | null {
